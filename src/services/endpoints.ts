import { ICompany, ICustomer, IFunding, IProfile, IRecomendation } from "../Interfaces/IRecomendation";
...
export const Api = {
  ...
  POST_CreateFundingRecommendation: async (
    payload: IFunding
  ): Promise<IResponseObject<any>> => {
    const response = await POST(
      `${DBServiceUrl}/Recomendations`,
  ...
  POST_CreateCustomerRecommendation: async (
    payload: ICustomer
  ): Promise<IResponseObject<any>> => {
    const response = await POST(
      `${DBServiceUrl}/Recomendations`,
